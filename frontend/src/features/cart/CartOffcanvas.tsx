...
export const CartOffcanvas = () => {
    ...
    const renderBody = () => {
        ...
        if (cartItems.length === 0) {
            return (
                <div className="text-center mt-5">
                    ...
                    <Button as={Link} to="/" variant="dark" onClick={closeCart}>
                        Ir a comprar
                    </Button>
                </div>
            );
        ...
        // Si hay items, mostrar la lista y el resumen
        return (
            <>
                {/* Lista de Items */}
                <div className="cart-items-list mb-3">
                    {cartItems.map(item => (
                        <Card key={item.id} className="mb-3 border-0 border-bottom rounded-0">
                            <Row g={0}>
                                <Col xs={4}>
                                ...
                                <Col xs={8}>
                                    <Card.Body className="py-0 pe-0">
                                        <Card.Title as="h6" className="mb-1">{item.variante.producto.nombre}</Card.Title>
                                        <Card.Text className="text-muted small">
                                            Talle: {item.variante.talle}
                                        </Card.Text>
                                        <Card.Text className="fw-bold mb-2">
                                            {formatPrice(item.variante.producto.precio_final)}
                                        </Card.Text>
                                        ...
                                        <div className="d-flex justify-content-between align-items-center">
                                            <input 
                                                ...
                                                style={{width: '60px'}}
                                            />
                                            <Button variant="link" size="sm" className="text-danger" onClick={() => removeItem(item.id)}>
                                                Eliminar
                                            </Button>
                                        </div>
                                    </Card.Body>
                ...
                <div className="cart-footer-summary">
                    ...
                    <Button variant="dark" className="w-100 mt-3" as={Link} to="/checkout" onClick={closeCart}>
                        Iniciar Pago
                    </Button>
                    <Button variant="outline-danger" size="sm" className="w-100 mt-2" onClick={clearCartItems}>
                        Vaciar Carrito
                    </Button>
                </div>
            </>
